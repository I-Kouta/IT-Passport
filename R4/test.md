...
- **帰納推論**は、個々の事例を基にして事例に共通する規則を得る方法であり意、得られた規則は**成立しないことがある**  
...
- NTP  
*Network Time Protocol*の略。ネットワークに接続されたサーバとクライアントのシステム時計を正しい時刻(協定世界時:UTC)に合わせるためのプロトコルです

- SMTP  
*Simple Mail Transfer Protocol*の略。インターネット環境において、クライアントからメールサーバに電子メールを送信したり、メールサーバ間で電子メールを転送したりするのに用いられるプロトコルです

- WPA  
*Wi-Fi Protected Access*の略。無線LANにおいてクライアント端末とアクセスポイントの間の通信を暗号化する規格です
